...
float colorchange(in float _color, in float _variansa){

    return mod(_color,_variansa); 
}
...
void main(){
    ...
    vec2 translate = vec2(cos(u_time), sin(u_time)); //Crea un tiempo que gira entre el eje [x,y], trasando circulos.
    Cardinal_Partician += translate * 0.35; 
    vec2 Cardinal_Partician_directcut = gl_FragCoord.xy / u_resolution; 



    color += vec3((cross(Cardinal_Partician, (0.25)))); 

    float colorx = colorchange(color.x, 2.0); 
    vec3 finalcolor = smoothstep(color, vec3(Cardinal_Partician.x, 0.00093 ,Cardinal_Partician.y), vec3(0.251, 0.6941, 0.6745)) + smoothstep(vec3 ( Cardinal_Partician.y), vec3(Cardinal_Partician.x), vec3( Cardinal_Partician_directcut.x , Cardinal_Partician_directcut.y, 0)); 
    ///code///
