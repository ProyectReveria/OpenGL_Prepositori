...
void main(){
    ...
    Cardinal_Partician -= vec2(0.5 * (sin(Two_pi) + sin(u_time)));

    Cardinal_Partician = rotate2d( sin(u_time)*sqrt(abs(pi)) ) * Cardinal_Partician;

    Cardinal_Partician += vec2(0.5);



    color += vec3(cross(Cardinal_Partician,0.4));


    ///code///

    gl_FragColor = vec4(color * sin(u_time),1.0);
    //Return gl_FragColor// 
