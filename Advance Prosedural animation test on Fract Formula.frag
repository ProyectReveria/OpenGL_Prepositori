...
//caracter here to restart:

vec3 calculation_hsb2rgb(in vec3 c){
    //color list
    vec3 negro = vec3(0.0);
    vec3 base = vec3(.0,2.0,4.0); 
    vec3 rojo = vec3(1.0,0.0,0.0);
    //color list
    vec3 rgb = clamp(abs(mod(c.x*6.0 + vec3(base), 6.0) -3.0)-1.0,0.0,1.0);
...
void main(){
    ...
    float Radio = length(tocenter) * 2.0; 
    float circuferencia = tan(angle); 

    //color = (calculation_hsb2rgb(vec3((log(circuferencia))+0.5,Radio,1.0)* u_time));
    color = (fract(sin(abs(calculation_hsb2rgb(vec3((log(circuferencia))+0.5,Radio,1.0)* length(st.x) * u_time)))));
    //color = calculation_hsb2rgb(vec3((circuferencia)+0.5,Radio,1.0));
    //color2 = calculation_hsb2rgb(vec3())
    //color = calculation_hsb2rgb(vec3((angle/Two_pi)+0.5,Radio,1.0)* u_time);

    //gl_FragColor = vec4(sin(color)/log(abs(pi)),1.0);
    gl_FragColor = vec4(color,0.9994);
}
